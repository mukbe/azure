...
void Buffer::CreateVertexBuffer(ID3D11Buffer ** buffer, void* vertex, UINT vertexSize)
{
	...
	HRESULT hr = _Device->CreateBuffer(&desc, &data, buffer);
	assert(SUCCEEDED(hr));
}
...
void Buffer::CreateTextureArray(ID3D11ShaderResourceView** pOut, vector<wstring>& fileNames, DXGI_FORMAT format, UINT filter, UINT mipFilter)
{
	...
	for (UINT i = 0; i < size; ++i)
	{
		...
		hr = D3DX11CreateTextureFromFile(_Device, fileNames[i].c_str(),
	...
	hr = _Device->CreateTexture2D(&texArrayDesc, 0, &texArray);
	assert(SUCCEEDED(hr));
	...
	for (UINT texElement = 0; texElement < size; ++texElement)
	{
		...
		for (UINT mipLevel = 0; mipLevel < texElementDesc.MipLevels; ++mipLevel)
		{
			...
			hr = _Context->Map(srcTex[texElement], mipLevel, D3D11_MAP_READ, 0, &mappedTex2D);
			assert(SUCCEEDED(hr));
			_Context->UpdateSubresource(texArray,
	...
	hr = _Device->CreateShaderResourceView(texArray, &viewDesc, pOut);
	assert(SUCCEEDED(hr));
