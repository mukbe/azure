...
void RenderTargetBuffer::CreateBuffer()
{
	...
	HRESULT hr;
	hr = _Device->CreateTexture2D(&texDesc, 0, &renderTargetTexture);
	assert(SUCCEEDED(hr));
	...
	hr = _Device->CreateRenderTargetView(renderTargetTexture, &rtvDesc, &rtv);
	assert(SUCCEEDED(hr));
	...
	hr = _Device->CreateShaderResourceView(renderTargetTexture, &srvDesc, &srv);
	assert(SUCCEEDED(hr));
}
...
void RenderTargetBuffer::CreateArrayBuffer()
{
	...
	hr = _Device->CreateShaderResourceView(renderTargetTexture, nullptr, &srv);
	assert(SUCCEEDED(hr));
}
